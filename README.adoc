...
The original developer identity remains recoverable from the author field when this setting is enabled.
====
...
Plugin authors that need to read a specific identity regardless of per-job configuration can use the flag-independent methods on `GitChangeSet`: `getGitAuthorName()`, `getGitAuthorEmail()`, `getGitAuthorDate()`, `getGitCommitterName()`, `getGitCommitterEmail()`, and `getGitCommitterDate()`.
